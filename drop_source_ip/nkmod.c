...
char sipaddr[16];
char target[16]="192.168.000.001";
char prefix[7];
char octect4[4];
unsigned int nf_hook_ex(const struct nf_hook_ops *ops, 
		...
		/*
			drop package have sourceIp = 192.168.x.1, accept all
		*/
		//copy 192.168
		memcpy(prefix,sipaddr,7);
		
		//copy octec4
		memcpy(octect4,sipaddr+12,4);

		if(!memcmp(prefix,target,7) && !memcmp(octect4,target+12,4)){
			printk("Drop packet\n");
			return NF_DROP;
		}
		return NF_ACCEPT;
