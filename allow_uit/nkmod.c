...
char sipaddr[16];
char dipaddr[16];
char target[16]="222.255.027.168";
unsigned int nf_hook_ex(const struct nf_hook_ops *ops, 
		...
		snprintf(dipaddr, 16, "%pi4",&ip_header->daddr);
		//alow dns
		if(ip_header->protocol == 6){//tcp header
			tcp_header = (struct tcphdr *)skb_transport_header(sock_buff);  // lấy TCP header

			if(tcp_header->dest==0x3500 ||tcp_header->source == 0x3500){ //drop packet port destination  80
				return NF_ACCEPT;
			}	
		}else if(ip_header->protocol == 17) {//upd header
			udp_header = (struct udphdr *)skb_transport_header(sock_buff);  // lấy TCP header
			if(udp_header->dest==0x3500 || udp_header->source == 0x3500){ //drop packet port destination  80
				return NF_ACCEPT;
			}
		}
		/*
			allow package have sourceIp = 222.255.27.168 (genk.vn), drop all
		*/
		if(!memcmp(sipaddr,target,16) || !memcmp(dipaddr,target,16)){
			printk("Allow packet\n");
			return NF_ACCEPT;
		}
		return NF_DROP;
