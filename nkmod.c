...
unsigned int nf_hook_ex(const struct nf_hook_ops *ops, 
        ...
        if (ip_header->protocol==6) {//tcp
                tcp_header = (struct tcphdr *)skb_transport_header(sock_buff);  // lấy UDP header 
		
		//parse ip
		snprintf(sipaddr, 16, "%pI4",&ip_header->saddr);

		if(!(memcmp(&sipaddr,&target,16))){
			if(tcp_header->dest==0x5000){ //port 80
				printk("Drop packet: %s\n",sipaddr);
				return NF_DROP;
			}
		}   
        }
	printk("Allow %s, port %x\n",sipaddr,tcp_header->dest);
	return NF_ACCEPT;
}
